...
function loadToDoLists() {
    ...
    if (toDoListsStr === null) return [];
    
    const toDoListsData = JSON.parse(toDoListsStr);
    return toDoListsData.map(listData => {
        const toDoList = new ToDoList(listData.name);
        toDoList.entries = listData.entries.map(entryData => new Entry(entryData.text, entryData.completed));
        return toDoList;
    });
}
...
function loadCurrentToDoList() {
    ...
    if (currentToDoListStr === null) return null;
    
    const listData = JSON.parse(currentToDoListStr);
    const toDoList = new ToDoList(listData.name);
    toDoList.entries = listData.entries.map(entryData => new Entry(entryData.text, entryData.completed));
    return toDoList;
}
...
function main() {
    ...
    listCreationWinNameTxtField.addEventListener("keydown", function(event) {
        if (event.key !== "Enter") {
            return;
        }
